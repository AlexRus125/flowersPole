...
async def get_trans(message: Message, bot: Bot, state: FSMContext):
    ...
    \n<b><em>Данная инструкция написана только для того, что надо прислать нам. Вам не нужно расставлять каждую цифру под соответствуйщей фоткой</em></b>''', parse_mode='html')
